...
REPO_URL = 'https://github.com/yicooooo/my-project.git'  # 替换为你的仓库地址
...
def _update_virtualenv(source_folder):
    virtualvenv_folder = source_folder + '/../virtualenv'
    if not exists(virtualvenv_folder + '/bin/pip'):
        run(f'python3.9 -m venv {virtualvenv_folder}')
    run(f'{virtualvenv_folder}/bin/pip install -r {source_folder}/requirements.txt')
...
if __name__ == "__main__":
    local('fab -f /path/fabfile.py deploy')  # 本地测试用